...
def initialize_openai_model() -> Model:
        ...
        try:
            ...
            model_instance = OpenAIServerModel(model_id=model_id,
            ...
            raise RuntimeError(f"Model initialization failed for model {model_id}: {e}")
...
def spawn_and_run_agent(
    ...
    """
    Dynamically creates, configures, and runs a new agent to perform a specific, self-contained task.
    This is the primary tool for an orchestrator to delegate work. The orchestrator must define the
    exact purpose, name, tools, and potentially managed agents for the agent it wants to create.

    Args:
        model: The language model instance the new agent will use.
        agent_name: A descriptive name for the agent to be spawned (e.g., "web_researcher", "sub_orchestrator").
        agent_description: A detailed description of the agent's specific purpose and capabilities.
        tools: A list of callable tool functions the new agent should have access to.
        directive: The detailed prompt, goal, or task to assign to the newly spawned agent.
        managed_agents: Optional. A list of pre-existing agent instances that the new agent will manage.
                        This is used to create sub-orchestrators.
        agent_config_overrides: Optional. A dictionary of parameters to override the default agent configuration.

    Returns:
        A string containing the final result from the spawned agent's run, to be used as an observation.

    Example (as would be generated by an orchestrator agent's LLM):
    ```python
    # Goal: Spawn a simple agent to perform a task.
    ...
    # Note: 'browse_webpage' and 'write_file' are assumed to be valid, imported tool functions.
    spawn_and_run_agent(
        model=self.model, # Assuming the parent agent has access to its own model
        agent_name="web_research_and_writer_agent",
        agent_description="A specialized agent that can browse a specific URL and write the findings to a file.",
        tools=[browse_webpage, write_file],
        directive="First, browse '[https://example.com/some_article](https://example.com/some_article)'. Then, write the content to 'research_summary.txt'.",
        managed_agents=[], # No managed agents for this simple worker
    ...
    try:
        ...
        spawned_agent = CodeAgent(
            name=agent_name,
            description=agent_description,
            tools=tools,
            model=model,
            managed_agents=managed_agents or [], # Pass the managed_agents list
            **config
        )
        logger.info(f"Successfully created agent '{agent_name}' with {len(tools)} tools and {len(managed_agents or [])} managed agents.")

    except Exception as e:
        logger.error(f"Failed to create agent instance '{agent_name}': {e}", exc_info=True)
        return f"Error: Could not instantiate agent '{agent_name}'. Reason: {str(e)}"

    # Step 2: Run the new agent with its directive.
    try:
        ...
        logger.error(f"Execution failed for spawned agent '{agent_name}': {e}", exc_info=True)
        return f"Error: The spawned agent '{agent_name}' failed during its run. Reason: {str(e)}"